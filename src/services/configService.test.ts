...
describe('ConfigService V2 - Multi-Dimensional Caching', () => {
  ...
  describe('Multi-Language Caching', () => {
    it('caches English and Spanish products separately', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/products')
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/products.es')
  ...
  describe('Multi-Bank Caching', () => {
    it('caches configurations for different banks separately', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/bank-info')

      // Load warm bank info
      const warmResult = await configService.getBankInfoFor('warmbank', 'en')
      expect(warmResult).toEqual(warmBankInfo)
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/warmbank/bank-info')
  ...
  describe('Combined Bank and Language Caching', () => {
    it('caches all four combinations separately: default/warm × en/es', async () => {
      const combinations = [
        { fi: null, lng: 'en', expectedUrl: 'http://localhost:3000/api/config/products' },
        { fi: null, lng: 'es', expectedUrl: 'http://localhost:3000/api/config/products.es' },
        { fi: 'warmbank', lng: 'en', expectedUrl: 'http://localhost:3000/api/config/warmbank/products' },
        { fi: 'warmbank', lng: 'es', expectedUrl: 'http://localhost:3000/api/config/warmbank/products.es' }
      ]
  ...
  describe('Preloading', () => {
    it('preloads both languages for a bank', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/warmbank/products')
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/warmbank/products.es')
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/warmbank/bank-info')
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3000/api/config/warmbank/bank-info.es')
    })
