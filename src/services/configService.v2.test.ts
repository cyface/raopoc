...
describe('ConfigService V2 - Multi-Dimensional Caching', () => {
  ...
  describe('Multi-Bank Caching', () => {
    it('caches configurations for different banks separately', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3001/api/config/bank-info')
  ...
  describe('Combined Bank and Language Caching', () => {
    it('caches all four combinations separately: default/warm × en/es', async () => {
      const combinations = [
        { fi: null, lng: 'en', expectedUrl: 'http://localhost:3001/api/config/products' },
        { fi: null, lng: 'es', expectedUrl: 'http://localhost:3001/api/config/products.es' },
        { fi: 'warmbank', lng: 'en', expectedUrl: 'http://localhost:3001/api/config/warmbank/products' },
        { fi: 'warmbank', lng: 'es', expectedUrl: 'http://localhost:3001/api/config/warmbank/products.es' }
      ]
  ...
  describe('Preloading', () => {
    it('preloads both languages for a bank', async () => {
      ...
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3001/api/config/warmbank/bank-info')
      expect(mockFetch).toHaveBeenCalledWith('http://localhost:3001/api/config/warmbank/bank-info.es')
    })
